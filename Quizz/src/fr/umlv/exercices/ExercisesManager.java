...
public class ExercisesManager {
	...
	//Correction of bugs in WatchService api
	 ...
	 * @param message
	 *            The message receive
	 * @return JsonObjet The JsonObject with the response
	...
	private JsonObject treatJsonExercice(JsonObject json, String name,String type) throws IOException {
		...
		case "ENTRY_MODIFY":
			...
			Exercice e = this.exercices.getOrGetAddExerciceById(name);
			//Reload file
			e = ExerciceFactory.createExercice(Paths.get("./" + name));
			JsonObject jsonTmp = new JsonObject(e.toJSon());
